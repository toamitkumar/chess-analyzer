                  ...
                  <tr *ngFor="let tournament of tournaments" 
                    ...
                    <td class="p-4 align-middle [&:has([role=checkbox])]:pr-0">
                      <div class="flex items-center gap-1">
                        <span class="font-medium">{{ calculateScore(tournament.wins, tournament.draws) }}</span>
                        <span class="text-xs text-muted-foreground">/ {{ tournament.gamesPlayed }}</span>
                      </div>
                    </td>
                    <td class="p-4 align-middle [&:has([role=checkbox])]:pr-0">
                      <span [class]="'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ' + 
                        (calculateWinRate(tournament.wins, tournament.gamesPlayed) > 50 ? 
                    ...
                    <td class="p-4 align-middle [&:has([role=checkbox])]:pr-0">
                      <div *ngIf="tournament.totalBlunders > 0" class="flex items-center gap-1 text-yellow-600">
...
export class Tournaments {
  ...
  getTotalGames(): number {
    return this.tournaments.reduce((sum, t) => sum + t.gamesPlayed, 0);
  }
  ...
  getAvgAccuracy(): string {
    const avgAccuracy = this.tournaments.reduce((sum, t) => sum + t.avgAccuracy, 0) / this.tournaments.length;
