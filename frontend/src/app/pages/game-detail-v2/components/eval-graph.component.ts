...
@Component({
  ...
  template: `
    <div class="eval-graph-container" *ngIf="moves.length > 0">
      <svg class="eval-graph" [attr.viewBox]="'0 0 ' + width + ' ' + height">
        ...
        <ng-container *ngFor="let move of moves; let i = index">
          <circle *ngIf="move.is_blunder" [attr.cx]="getGraphX(i)" [attr.cy]="getGraphY(move.evaluation)" r="5" fill="#db3434" class="eval-dot" (click)="onMoveClick(i)"/>
          <circle *ngIf="move.is_mistake && !move.is_blunder" [attr.cx]="getGraphX(i)" [attr.cy]="getGraphY(move.evaluation)" r="5" fill="#e69f00" class="eval-dot" (click)="onMoveClick(i)"/>
          <circle *ngIf="move.is_inaccuracy && !move.is_mistake && !move.is_blunder" [attr.cx]="getGraphX(i)" [attr.cy]="getGraphY(move.evaluation)" r="4" fill="#56b4e9" class="eval-dot" (click)="onMoveClick(i)"/>
        </ng-container>
